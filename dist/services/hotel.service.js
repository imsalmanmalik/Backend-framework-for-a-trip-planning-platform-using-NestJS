...
const hotel_entity_1 = require("../models/hotel.entity");
let HotelService = HotelService_1 = class HotelService {
    constructor(hotelRepository) {
        this.hotelRepository = hotelRepository;
        this.logger = new common_1.Logger(HotelService_1.name);
    }
    async findAll() {
        try {
            const findHotels = await this.hotelRepository.find({});
            if (!findHotels) {
    ...
    async create(data) {
        try {
            const saveHotel = await this.hotelRepository.save(data);
            if (!saveHotel) {
...
HotelService = HotelService_1 = __decorate([
